...
<main class="sky-section">
    <div class="sky-container form-shell">
        <section class="premium-card">
            ...
            @if($order->isExpired())
                <div class="alert alert-warning" style="margin:22px 0;">
                    Cette commande a expire. Veuillez creer une nouvelle commande pour continuer.
                </div>
            @endif
        ...
        <aside class="premium-card">
            ...
            @if($order->expires_at)
                <p><strong>Expiration :</strong> {{ $order->expires_at->format('d/m/Y H:i') }}</p>
            @endif
        </aside>
    </div>
</main>
